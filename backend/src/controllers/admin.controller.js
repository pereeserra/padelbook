...
// Controlador per actualitzar el rol d'un usuari (per a l'administrador)
exports.updateUserRole = async (req, res) => {
  try {
    const userId = parsePositiveInteger(req.params.id);
    const rol = typeof req.body?.rol === "string" ? req.body.rol.trim().toLowerCase() : "";
    ...
    if (!["usuari", "admin"].includes(rol)) {
      return res.status(400).json({
        error: "El rol indicat no és vàlid",
      });
    }

    if (req.user.id === userId) {
      return res.status(400).json({
        error: "No pots canviar el teu propi rol des d'aquest panell",
      });
    }

    const [existingUsers] = await db.query(
      "SELECT id, nom, email, rol FROM users WHERE id = ? LIMIT 1",
      [userId]
    );
    ...
    if (targetUser.rol === rol) {
      return res.status(400).json({
        error: "L'usuari ja té aquest rol",
      });
    }
    ...
    await db.query(
      ...
      [
        ...
        "user",
        userId,
        `L'admin ha canviat el rol de l'usuari "${targetUser.nom}" (${targetUser.email}) a "${rol}"`,
      ]
    );
    ...
    return res.status(500).json({
      error: "Error actualitzant el rol de l'usuari",
    });
  }
};
  ...
  try {
    ...
    await db.query(
      ...
      [
        ...
        "user",
        userId,
        `Canvi de rol a "${rol}" per l'usuari ID ${userId}`,
      ]
    );
